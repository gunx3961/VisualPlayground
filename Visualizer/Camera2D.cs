...
namespace Visualizer
{
    public struct Camera2D
    {
        ...
        public Camera2D(double defaultPpu)
        {
            PixelCenter = default;
        ...
        {
            PixelCenter = Point.Zero + new Point(window.ClientBounds.Width / 2, window.ClientBounds.Height / 2);
            PixelPerUnit = DefaultPpu;
        }

        public Matrix GetMatrix(GameWindow window)
        {
            return Matrix.CreateTranslation(-PixelCenter.X + window.ClientBounds.Width / 2,
                -PixelCenter.Y + window.ClientBounds.Height / 2, 0);
        }

        public Vector2 ScreenSpacePointToUnit(GameWindow window, Point screenSpacePosition)
        {
            var cameraMatrix = GetMatrix(window);
        ...
        public void BatchBegin(GameWindow window, SpriteBatch batch)
        {
            var cameraMatrix = GetMatrix(window);
